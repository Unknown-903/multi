...
class Progress:
    ...
    async def ProgressBar(self, current, total, ud_type, start):
        chat_id = self._mess.chat.id
        mes_id = self._mess.id
        ...
        if round(diff % float(3)) == 0 or current == total:
            ...
            PROGRESS_BOX[f"{chat_id}_{mes_id}"] = {
...
@Client.on_callback_query(
    filters.create(lambda _, __, query: query.data.startswith("filedlprog"))
)
async def file_dl_progres(c, m):
    ...
    try:
        await m.answer(
            text=p_message.format(
                **PROGRESS_BOX[f"{chat_id}_{message_id}"]
            ),
            show_alert=True,
        )
