...
@Client.on_callback_query(filters.regex("^mpthreege"))
async def mp3_audio(c, m):
    ...
    if bool:
        return
    '''
    bool = await CH.audio_dur_limito(c, m)
    if bool:
        return
    '''
    ...
    if bool:
        return
    '''
    bool = await CH.audio_dur_limit(c, m, msg, duration, input)
    if bool:
        return
    '''
    if reply_msg.media:
    ...
    try:
        ...
        captions = "File Name : " + new_file_name + ".mp3"

    ft = "Audio Converted to MP3 ✅ "
