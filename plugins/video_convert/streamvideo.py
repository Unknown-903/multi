...
@Client.on_callback_query(filters.regex("^streamvid"))
async def streamvi_(c, m):
        ...
        try:
            ...
            if Config.LOG_CHANNEL:
                try:
                    ...
                    print(e)
            logger.info(
                f" video to file converted ✅ By {m.from_user.first_name} {str(m.from_user.id)} @{m.from_user.username}"
            )
        except Exception as e:
            print(e)
            await msg_edit(msg, f"⚠️ **Error** : {e}")
