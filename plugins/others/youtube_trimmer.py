...
async def video_process(
    ...
    output = output_directory + "/" + new_file_name + f".mkv"
...
@Client.on_callback_query(filters.regex("^yttrim")) 
async def yttrimmer(bot, update):
    ...
    logger.info(
        f"Video start time is 👉 {start_time} For user {update.from_user.id} @{update.from_user.username}"
    )
    ...
    logger.info(
        f"Video End time is 👉 {end_time} For user {update.from_user.id} @{update.from_user.username}"
    )
    ...
    try:
        video_file = await video_process(
            file_url, Config.DOWNLOAD_LOCATION, start_time, end_time, cfile_name
        )
    except Exception as e:
        await clear_server(user_id, save_ytdl_json_path)
        await update.message.reply(
