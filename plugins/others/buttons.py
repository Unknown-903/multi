...
@Client.on_callback_query(filters.regex("^capbutton"))
async def captionbutton_(bot, update):
    ...
    if number_asks == "1":
        ...
        if file_name.lower() == "/cancel":
            await update.message.reply_text(
                "Process Cancelled  ✅", reply_to_message_id=reply_msg.message_id
            )
            return
    ...
    try:
        await reply_msg.copy(update.message.chat.id, reply_markup=reply_markups)
        logger.info(
...
@Client.on_callback_query(filters.regex("^removebutton"))
async def removebuttons_(bot, update):
    await update.message.delete()
    update.message.chat.id
    reply_msg = update.message.reply_to_message
    if update.from_user.id not in Config.AUTH_USERS:
    ...
    try:
        await reply_msg.copy(update.message.chat.id, reply_markup=None)
        logger.info(
            f"Successfully Buttons Removed ✅ By {update.from_user.first_name} {str(update.from_user.id)} @{update.from_user.username}"
        )

        if Config.LOG_CHANNEL:
            cad = await reply_msg.copy(chat_id=Config.LOG_CHANNEL, reply_markup=None)
            await cad.reply_text(
                f"𝐔𝐬𝐞𝐫 𝐈𝐧𝐟𝐨𝐫𝐦𝐚𝐭𝐢𝐨𝐧 :\n\n🌷 **First Name :** `{update.from_user.first_name}`\n\n🌷 **User Id :** `{update.from_user.id}`\n\n🌷 **User Name :** `@{update.from_user.username}`\n\n🌷 Used Buttons Remover ✅"
            )
    except Exception as e:
        print(e)
        await update.message.reply(
