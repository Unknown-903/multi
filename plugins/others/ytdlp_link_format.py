...
@Client.on_callback_query(filters.regex("^ydlink"))
async def ydlink_(bot, update):
    ...
    if reply_msg.empty:
        await update.message.reply(
            "⚠️ File not found!", reply_to_message_id=reply_msg.id
        )
        return
    ...
    if t_response:
        ...
        if "formats" in response_json:
            for formats in response_json["formats"]:
                ...
                cb_ytdata = "mkydlink/{}/{}/{}".format("link", format_id, format_ext)
                    ...
                    if (yt_acodec == "mp4a.40.2") or (yt_acodec != "none"):
                        ikeyboard = [
                            InlineKeyboardButton(
                                ...
                                + format_ext
                                + " ",
                                callback_data=(cb_ytdata).encode("UTF-8"),
                            )
                        ]
                ...
                if yt_resolution == "audio only":
                    if (
                        ...
                        ikeyboard = [
                            InlineKeyboardButton(
                                f"🎵 {audio_bit_rate}k - {yt_resolution} ({format_ext}) {approx_file_size}",
                                callback_data=(cb_ytdata).encode("UTF-8"),
                            )
                        ]
                        inline_keyboard.append(ikeyboard)
            ...
            inline_keyboard.append(
                [
                    InlineKeyboardButton(
                        "Make Direct Link", callback_data=(cb_ytdata).encode("UTF-8")
                    )
                ]
            )
            cb_ytdata = "mkydlink/{}/{}/{}".format("link", format_id, format_ext)
        ...
        try:
            await bot.send_message(
                ...
                + f"\n\n⏰ **Total Duration -** {video_duration}\n\n👉 For making Direct link Click on Below Keyboard",
                reply_markup=reply_markup,
                reply_to_message_id=reply_msg.id,
            )
        except:
            await clear_server(user_id, save_ytdl_json_path)
            await db.get_yfilter(update.from_user.id)
            await bot.send_message(
                chat_id=update.message.chat.id,
                text=f"🤦‍♂️ I am not finding Formats\n\n👉 Try another links",
                reply_to_message_id=reply_msg.id,
            )
