...
@Client.on_callback_query(filters.regex("^threeaudio"))
async def merge_three_(bot, update):
    ...
    if reply_msg.media:
        logger.info(
            f"☘️ Sent file for Merging Audio 3. User {update.from_user.id} @{update.from_user.username}"
        )
        ...
        if request_aud.media:
            try:
                bca = await bc.edit(f"**Downloading....**")
            except:
                await delete_msg(bc)
                try:
        ...
        if third_audio.media:
            ...
            real_audio = await bot.download_media(
                ...
                progress=progress_bar.progress_for_pyrogram,
                progress_args=("**Downloading Audio-3....**", c_time),
            )
            ...
            try:
                bc = await bcc.edit(f"Audio-3 Downloaded Successfully ✅")
            except:
    ...
    try:
        cd = await bc.edit(f"**3 Audios Merging....**")
    except:
        await delete_msg(bc)
        try:
            cd = await update.message.reply(
                "**3 Audios Merging....**", reply_to_message_id=reply_msg.id
            )
        except:
            await clear_server(
                user_id, saved_file_path, third_audio_path, audio_file_path
            )
            logger.info(
    ...
    try:
        ...
        await clear_server_two(third_audio_path)
        print(e)
