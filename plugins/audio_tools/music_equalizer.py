...
@Client.on_callback_query(filters.regex("^super_eq"))
async def equalizers(c, m):
    ...
    try:
        bassaudio = await db.get_bassaudio(m.from_user.id)
    except:
        bassaudio = 5

    try:
        trebleaudio = await db.get_trebleaudio(m.from_user.id)
    except:
        trebleaudio = 5

    try:
        audio_speeds = await db.get_audio_speed(m.from_user.id)
        audio_speed = int(audio_speeds)
        audio_speed = audio_speed / 100
    except:
        audio_speed = 1

    try:
        audio_volume = await db.get_audio_vol(m.from_user.id)
        audio_vol = int(audio_volume)
        audio_vol = audio_vol / 100
    except:
        audio_vol = 1
    ...
    commands = [
        ...
        input,
        "-filter_complex",
        f"bass=g={bassaudio}, treble=g={trebleaudio}, atempo={audio_speed}, volume={audio_vol}",
        "-b:a",
        f"{audio_bitrate}K",
        output_directory,
    ]
