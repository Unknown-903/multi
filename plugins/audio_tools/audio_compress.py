...
@Client.on_callback_query(filters.regex("^compressaud"))
async def audio_compress(c, m):
    ...
    try:
        compress_range = await db.get_audio_quality(m.from_user.id)
    except:
        compress_range = 5
    ...
    if reply_msg.media:
        try:
            size_input = os.path.getsize(real_audio)
            captions = (
                "File Name : "
                + new_file_name
                + f".mp3"
                + f"\n\nOrignal Size : {humanbytes(size_input)}\nNow Audio Size : {humanbytes(output_size)}\nCompressed Range : {compress_range}"
            )
        except:
